...
class PriceAnalysis:
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "trend_direction": self.trend_direction.value if hasattr(self.trend_direction, 'value') else str(self.trend_direction),
...
class RealDatabaseAdapter:
    ...
    def get_price_history_from_db(self, product_id: str, days: int = 30) -> List[float]:
            ...
            try:
                ...
                results = self._session.query(PriceHistory.current_price)\
    ...
    def get_product_info_from_db(self, product_id: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            product_info_with_latest_price = self._session.query(Product, PriceHistory)\
    ...
    def get_all_products_from_db(self) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            results = self._session.query(
                ...
                Product.name,
                Product.site
            ).distinct(Product.product_id).all() # Get unique products with Distinct
...
class PriceAnalyzer:
    ...
    def _determine_alert_level(self, change_percent: float, 
        ...
        if (change_percent <= -10) or \
           (current_price < average_price * 0.85) or \
           (current_price <= minimum_price * 1.02): 
        ...
        for product_id in product_ids:
            ...
            if analysis and analysis.strategies:
                for strategy in analysis.strategies:
                    print(f"  {strategy['name']}: {strategy['trend']} (confidence: {strategy['confidence']:.2f})")
            
            if analysis:
                all_analyses.append(analysis)
                print(f"  Product: {analysis.product_name}")
                print(f"  Current Price: {analysis.current_price:.2f} {analysis.currency}")
