...
class AnalysisDataLoader:
    ...
    def load_analysis_data(
        file_path: str = "analysis_32780.json",
    ...
    def load_visualization_data(
        file_path: str = "visualization_data.json",
    ) -> Optional[Dict[str, Any]]:
...
class PriceChartVisualizer:
    ...
    def create_price_history_chart(
        self, df: pd.DataFrame, product_name: str
    ) -> go.Figure:
        ...
        fig.update_layout(
            ...
            xaxis=dict(title="Date", tickformat="%Y-%m-%d", gridcolor="lightgray"),
            yaxis=dict(title="Price (TL)", tickprefix="₺", gridcolor="lightgray"),
            hovermode="x unified",
    ...
    def create_price_distribution_chart(self, df: pd.DataFrame) -> go.Figure:
        ...
        fig.update_layout(height=500, template="plotly_white", bargap=0.1)
    ...
    def create_analysis_summary_chart(self, analysis_data: Dict[str, Any]) -> go.Figure:
        ...
        fig.update_layout(
            ...
            yaxis=dict(title="Price (TL)", tickprefix="₺"),
            template="plotly_white",
            height=500,
        )
    ...
    def create_trend_indicator(self, analysis_data: Dict[str, Any]) -> go.Figure:
        ...
        fig.update_layout(
            title=f"Price Trend: {analysis_data['product_name']}", height=400
        )

        return fig

    def price_with_moving_average(
        self, df: pd.DataFrame, product_name: str, window: int = 7
    ) -> go.Figure:
...
class PriceTableVisualizer:
    ...
    def create_price_history_table(
        self, df: pd.DataFrame, product_name: str = None
    ) -> str:
    ...
    def create_summary_table(self, df: pd.DataFrame) -> str:
        ...
        summary = (
            ...
            .agg({"current_price": ["min", "max", "mean", "count"]})
            .round(2)
        )
        ...
        return tabulate(summary, headers="keys", tablefmt="fancy_grid", floatfmt=".2f")
...
class PriceVisualizer:
    ...
    def show_price_history(self, product_name: str = None):
        ...
        if product_name:
            ...
            fig_ma = self.chart_viz.price_with_moving_average(
                self.price_df, product_name
            )
            if fig_ma:
                fig_ma.show()
